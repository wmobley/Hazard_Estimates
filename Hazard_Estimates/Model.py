    ...
    def predict(self, data):
        ...
        for category in data.X_[self.split_model].unique():
            ...
            if len(data.X_.loc[X_loc_]) > 0:
                try:
                    print("subdivided:" ,category, data.X_.loc[X_loc_].occupancy.unique())
        ...
        if self.Dynamic_rasters !=None:
            for key in self.Dynamic_rasters:
                for time in key['time']:
                    print( self.variable_year_range, time)
                    self.variable_year_range.append(time)
        self.variable_year_range = np.unique(self.variable_year_range).tolist()